...
@app.function(
    image=image, gpu="T4", timeout=600, container_idle_timeout=120, memory=8192  # 8GB memory for model loading
)
@modal.asgi_app()
def fastapi_app():
            ...
            try:
                ...
                try:
                    ...
                    wav, sr, spect = f5tts.infer(
                        ref_file=ref_path, ref_text=ref_text if ref_text else "", gen_text=text
                    )
...
"""
                    ...
                    result = subprocess.run(
                        ["python", script_path], cwd="/app/F5-TTS", capture_output=True, text=True, timeout=300
                    )
    ...
    async def health_check():
        return {"status": "healthy", "model": "F5-TTS", "cuda_available": torch.cuda.is_available()}
