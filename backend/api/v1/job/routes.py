...
def list_jobs():
    ...
    try:
        ...
        if status and status not in valid_statuses:
            return error_response(f"Invalid status. Must be one of: {', '.join(valid_statuses)}", "INVALID_STATUS")

        valid_sort_fields = ["created_at", "updated_at", "status", "progress", "duration"]
        if sort_by not in valid_sort_fields:
            return error_response(
                f"Invalid sort_by field. Must be one of: {', '.join(valid_sort_fields)}", "INVALID_SORT_FIELD"
            )
        ...
        try:
            ...
            for job in jobs:
                ...
                if not include_text:
                    if len(job_dict["text_content"]) > 100:
                        job_dict["text_content"] = job_dict["text_content"][:100] + "..."
            ...
            meta = {
                ...
                "filters": {"status": status, "voice_model_id": voice_model_id, "text_search": text_search},
    ...
    try:
        ...
        try:
            ...
            voice_model = (
                ...
                .filter(VoiceModel.id == data["voice_model_id"], VoiceModel.is_active.is_(True))
                .first()
            )

            if not voice_model:
                return error_response("Voice model not found or inactive", "VOICE_MODEL_NOT_FOUND", status_code=404)
            ...
            if existing_job:
                return success_response(
                    existing_job.to_dict(), message="Duplicate synthesis job already exists", status_code=200
                )
            ...
            return success_response(job.to_dict(), message="Synthesis job created successfully", status_code=201)
...
def update_job(job_id):
    ...
    try:
        ...
        try:
            ...
            return success_response(
                job.to_dict(), message=f"Job updated successfully. Updated fields: {', '.join(updated_fields)}"
            )
        ...
        try:
            ...
            if "status" in data:
                ...
                if new_status not in valid_statuses:
                    return error_response(
                        f"Invalid status. Must be one of: {', '.join(valid_statuses)}", "INVALID_STATUS"
                    )
...
def generate_job_progress_events(job_id):
    ...
    try:
        ...
        error_data = {"error": "Stream error occurred", "code": "STREAM_ERROR", "details": str(e), "job_id": job_id}
...
def stream_job_progress(job_id):
    ...
    try:
        ...
        return error_response(f"Failed to start progress stream: {str(e)}", "STREAM_ERROR", status_code=500)
