...
def create_voice_clone():
    ...
    if not sample_ids or len(sample_ids) == 0:
        return jsonify({"success": False, "error": "At least one sample_id is required"}), 400
    ...
    try:
        ...
        with db.get_session() as session:
            samples = (
                ...
                .filter(VoiceSample.id.in_(sample_ids), VoiceSample.user_id == user_id, VoiceSample.status == "ready")
                .all()
            )

            if len(samples) != len(sample_ids):
                return jsonify({"success": False, "error": "Some samples not found or not ready"}), 400

            # Use the first sample as primary reference
            primary_sample = samples[0]
            if not os.path.exists(primary_sample.file_path):
                return jsonify({"success": False, "error": "Primary sample file not found"}), 400
        ...
        if not is_valid:
            return jsonify({"success": False, "error": f"Audio validation failed: {validation_message}"}), 400
        ...
        return jsonify({"success": False, "error": f"Failed to create voice clone: {str(e)}"}), 500
...
def list_voice_clones():
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Failed to list voice clones: {str(e)}"}), 500
    ...
    try:
        ...
        with db.get_session() as session:
            ...
            voice_model = (
                session.query(VoiceModel)
                .join(VoiceSample)
                .filter(VoiceModel.id == clone_id, VoiceSample.user_id == user_id, VoiceModel.model_type == "f5_tts")
                .first()
            )

            if not voice_model:
                return jsonify({"success": False, "error": "Voice clone not found"}), 404
        ...
        return jsonify({"success": False, "error": f"Failed to get voice clone: {str(e)}"}), 500
    ...
    try:
        ...
        with db.get_session() as session:
            ...
            try:
                ...
                print(f"Failed to delete clone from F5-TTS service: {type(e).__name__}: {str(e)}")

            # Delete from database
            session.delete(voice_model)
            session.commit()

            return jsonify({"success": True, "data": {"message": "Voice clone deleted successfully"}})

    except Exception as e:
        return jsonify({"success": False, "error": f"Failed to delete voice clone: {str(e)}"}), 500
...
def select_voice_clone(clone_id: str):
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Failed to select voice clone: {str(e)}"}), 500
...
def synthesize_with_clone(clone_id: str):
    ...
    if not data or "text" not in data:
        return jsonify({"success": False, "error": "Text is required for synthesis"}), 400
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Failed to synthesize speech: {str(e)}"}), 500
