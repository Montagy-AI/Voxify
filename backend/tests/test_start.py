#!/usr/bin/env python3
"""
Test suite for start.py - Backend Server Initialization
Tests start.py as a script without importing it directly
Uses subprocess calls to test actual behavior
"""
...
def get_start_py_path():
    """Find the start.py file in Docker-aware way"""
    current_dir = os.path.dirname(__file__)
    start_py_path = os.path.abspath(os.path.join(current_dir, "..", "start.py"))
...
class TestStartScriptExecution:
    ...
    def test_start_init_only_success(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Should exit successfully (code 0) or with our expected success behavior
    ...
    def test_start_seed_only_with_mock_database(self):
        ...
        # But it should fail gracefully with our error handling
        assert "Welcome to Voxify!" in result.stdout
    ...
    def test_start_skip_flags_init_only(self):
        ...
        # Should succeed since we're skipping initialization and just doing init-only
...
class TestFileStorageCreation:
    ...
    def test_file_storage_directories_created(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Run start.py init-only in the temp directory
            result = subprocess.run([
                sys.executable, start_py_path, "--init-only"
            ], capture_output=True, text=True, timeout=60,
            cwd=temp_dir, env=env)
    ...
    def test_file_storage_environment_variables_mentioned(self):
        ...
        # Should mention file storage initialization
...
class TestStartupConfiguration:
    ...
    def test_environment_variables_recognition(self):
        ...
        # Use init-only to avoid actually starting server
        result = subprocess.run([
            sys.executable, start_py_path, "--init-only"
        ], capture_output=True, text=True, timeout=30, env=env)
...
class TestErrorHandling:
    ...
    def test_missing_dependencies_graceful_failure(self):
        ...
        start_py_path = get_start_py_path_or_skip()
        # Try to run without proper database setup - should fail gracefully
        env = os.environ.copy()
...
def run_tests():
    """Run tests using pytest - matches your existing test pattern"""
    pytest.main([__file__, "-v"])


def test_configuration():
    """Test that the configuration is correct - matches your existing pattern"""
    import platform
    
    start_py_path = get_start_py_path()
    print(f"Start.py path: {start_py_path}")
    print(f"Start.py exists: {start_py_path and os.path.exists(start_py_path)}")
    print(f"Platform: {platform.system()}")
    print(f"Python executable: {sys.executable}")
    
    # Test that start.py is accessible
    if start_py_path and os.path.exists(start_py_path):
        print("start.py: Available")
        print(f"start.py size: {os.path.getsize(start_py_path)} bytes")
    else:
        print("start.py: Not found")
        current_dir = os.path.dirname(__file__)
        print(f"Current test directory: {current_dir}")
        print(f"Current working directory: {os.getcwd()}")
        if os.path.exists(current_dir):
            files = [f for f in os.listdir(current_dir) if f.endswith('.py')]
            print(f"Python files in test directory: {files}")
        
        # Show what directories we checked
        print("Directories checked for start.py:")
        possible_paths = [
            os.path.join(current_dir, "..", "start.py"),
            os.path.join(current_dir, "start.py"),
        ...
        try:
            ...
            print(f"Files in current working directory: {cwd_files[:10]}...")  # First 10 files
        except:
            print("Could not list current working directory")
