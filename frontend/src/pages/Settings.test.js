    ...
    describe('Last Login Display', () => {
        test('displays formatted last login date when available', async () => {
            ...
            await waitFor(() => {
                ...
                expect(screen.getByText(/1\/15\/2025, \d{1,2}:\d{2}:\d{2} (AM|PM)/)).toBeInTheDocument();
            });
        });
        ...
        test('displays "Never" when last login is null', async () => {
            ...
            authService.getUserProfile.mockResolvedValue({ data: profileWithoutLogin });
            render(<Settings />);
            await waitFor(() => {
        ...
        test('displays "Never" when last login is empty string', async () => {
            ...
            authService.getUserProfile.mockResolvedValue({ data: profileWithEmptyLogin });
            render(<Settings />);
            await waitFor(() => {
                expect(screen.getByText('Never')).toBeInTheDocument();
            });
    ...
    */
    describe('Data Handling Edge Cases', () => {
        ...
        test('handles partial profile data', async () => {
            ...
            render(<Settings />);
            await waitFor(() => {
                expect(screen.getByDisplayValue('partial@test.com')).toBeInTheDocument();
