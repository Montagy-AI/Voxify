    ...
    */
	describe('Form Submission', () => {
		...
		test('Successful registration and login redirects to home', async () => {
			...
			await waitFor(() => {
				expect(authService.login).toHaveBeenCalledWith(validFormData.email, validFormData.password);
			});
			expect(mockNavigate).toHaveBeenCalledWith('/');
		});
		...
		test('Show error message when registration fails', async () => {
			authService.register.mockResolvedValue({ success: false, error: 'Email already exists' });
			render(<RegisterWithRouter />);
			fillForm();
			fireEvent.click(screen.getByRole('button', { name: /create account/i }));
			await waitFor(() => {
		...
		test('Show loading state during registration', async () => {
			...
			authService.register.mockImplementation(() => new Promise(
                resolve => setTimeout(() => resolve({ success: true }), 100)
            ));
			authService.login.mockResolvedValue({ success: true });
			render(<RegisterWithRouter />);
			fillForm();
		...
		test('Clear error message when form is resubmitted', async () => {
			authService.register.mockResolvedValueOnce({ success: false, error: 'Email already exists' })
