...
describe('HelpPage', () => {
  ...
  describe('Rendering and Content', () => {
    ...
    test('Render all section headings', () => {
      renderWithRouter(<HelpPage />);
      expect(
        screen.getByRole('heading', { name: /what is voxify\?/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole('heading', { name: /using the dashboard/i })
      ).toBeInTheDocument();
      expect(
    ...
    test('Display Voxify description correctly', () => {
      renderWithRouter(<HelpPage />);
      expect(
        screen.getByText(/voxify is an ai-powered platform for voice cloning/i)
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          /this project focuses on building the core infrastructure/i
        )
      ).toBeInTheDocument();
    });
    ...
    test('Render dashboard usage instructions', () => {
      renderWithRouter(<HelpPage />);
      expect(
        screen.getByText(/from your dashboard, you can:/i)
      ).toBeInTheDocument();
  ...
  describe('Button Functionality', () => {
    ...
    test('Check buttons have correct styling classes', () => {
      ...
      expect(cloneButton).toHaveClass(
        'bg-black',
        'text-white',
        'px-6',
        'py-3',
