...
describe('Tasks Component', () => {
	...
	describe('Jobs Loading and Display', () => {
		...
		test('displays empty state when no jobs exist', async () => {
			...
			await waitFor(() => {
				expect(screen.getByText('No generated voices found')).toBeInTheDocument();
			});
		});
		...
		test('handles job loading failure with general error', async () => {
			...
			await waitFor(() => {
				expect(screen.getByText('Failed to load synthesis jobs. Please try again later.')).toBeInTheDocument();
			});
		});
	});
