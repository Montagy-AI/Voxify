...
describe('VoiceClone Component', () => {
	...
	describe('File Upload', () => {
		test('successful file upload updates uploaded samples list', async () => {
			...
			expect(global.alert).toHaveBeenCalledWith('Successfully uploaded 1 file(s)');
		});
		...
		test('handles upload failure with error alert', async () => {
			...
			await waitFor(() => {
				expect(global.alert).toHaveBeenCalledWith('Failed to upload test-audio.wav: Upload failed');
			});
		});

		test('handles upload exception with error alert', async () => {
			voiceCloneService.uploadVoiceSample.mockRejectedValue(new Error('Network error'));
			...
			await waitFor(() => {
				expect(global.alert).toHaveBeenCalledWith('Failed to upload test-audio.wav: Network error');
			});
		});
	...
	describe('Voice Clone Creation', () => {
		...
		test('successful voice clone creation navigates to dashboard', async () => {
			...
			await waitFor(() => {
				expect(global.alert).toHaveBeenCalledWith('Voice clone created successfully! 🎉');
			});
		...
		test('handles voice clone creation failure', async () => {
			...
			await waitFor(() => {
				expect(global.alert).toHaveBeenCalledWith('Creation failed: Creation failed');
			});
	...
	describe('Component Styling', () => {
		test('renders with correct CSS classes', () => {
			...
			const container = document.querySelector('.min-h-screen.bg-black.text-white.p-8');
			expect(container).toBeInTheDocument();
			
			const form = document.querySelector('form.space-y-8');
