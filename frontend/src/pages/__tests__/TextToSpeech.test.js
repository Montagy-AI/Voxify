...
describe('TextToSpeech Component', () => {
	...
	describe('Initial Render', () => {
		...
		test('loads voice clones on mount', async () => {
			...
			await waitFor(() => {
				expect(screen.getByText('John Voice (en-US) - ready')).toBeInTheDocument();
				expect(screen.getByText('Jane Voice (en-GB) - processing')).toBeInTheDocument();
			});
		});
	...
	describe('Voice Clone Information Display', () => {
		...
		test('does not show voice clone info when no voice is selected', async () => {
			...
			expect(screen.queryByText('Selected Voice Clone')).not.toBeInTheDocument();
		});
	...
	describe('Voice Clone Loading States', () => {
		...
		test('handles voice clone loading failure', async () => {
			...
			voiceCloneService.listVoiceClones.mockRejectedValue(new Error('Network error'));
	...
	describe('Speech Generation', () => {
		test('successful speech generation shows generated audio player', async () => {
			...
			voiceCloneService.synthesizeWithClone.mockResolvedValue(mockSynthesizeResponse);
		...
		test('handles speech generation failure', async () => {
			...
			await waitFor(() => {
				expect(global.alert).toHaveBeenCalledWith('Failed to generate speech: Generation failed');
			});
		});
	});
	...
	describe('Error Handling', () => {
		test('logs error when voice clone loading fails', async () => {
			const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
			voiceCloneService.listVoiceClones.mockRejectedValue(new Error('API Error'));
	...
	describe('Configuration Controls', () => {
		...
		test('all settings controls are disabled during generation', async () => {
			...
			voiceCloneService.synthesizeWithClone.mockImplementation(
				() => new Promise(resolve => setTimeout(() => resolve(mockSynthesizeResponse), 100))
			);
	...
	describe('Generated Audio Player', () => {
		...
		test('displays generated audio information correctly', async () => {
			...
			// Check audio information display within the generated audio section
			const generatedAudioSection = screen.getByText('Generated Audio').closest('div');
			expect(generatedAudioSection).toBeInTheDocument();
			
			// Check for text within the audio info (not the input field)
			expect(generatedAudioSection.querySelector('[data-testid="audio-text"]') || 
				   screen.getAllByText('Hello world test')[1]).toBeInTheDocument();
			expect(screen.getByText('Voice Clone')).toBeInTheDocument();
			expect(screen.getAllByText('John Voice')[1]).toBeInTheDocument(); // Second instance in audio info
			expect(screen.getAllByText('en-US')[1]).toBeInTheDocument(); // Second instance in audio info
		});
		...
		test('truncates long text in audio information display', async () => {
			...
			const longText = 'This is a very long text that should be truncated when displayed in the audio information section because it exceeds 100 characters in length.';
		...
		test('new generation button functionality', async () => {
			...
			const newGenerationButton = screen.getByText('Generate New').closest('button');
	...
	describe('Audio Error Handling', () => {
		test('audio player renders correctly with controls', async () => {
			...
			// Check that hidden audio element exists
