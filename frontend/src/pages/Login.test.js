...
describe('Login Component', () => {
	...
	describe('Form Submission', () => {
		...
		const fillForm = () => {
			fireEvent.change(screen.getByLabelText('Email'), { target: { value: validCredentials.email } });
			fireEvent.change(screen.getByLabelText('Password'), { target: { value: validCredentials.password } });
		};
		...
		test('Clear error message when form is resubmitted', async () => {
			authService.login.mockResolvedValueOnce({ success: false, error: 'Invalid credentials' })
	...
	describe('Error Handling', () => {
		test('Error message is displayed with proper styling', async () => {
			authService.login.mockResolvedValue({ success: false, error: 'Account locked' });
			render(<LoginWithRouter />);
