...
describe('API Configuration', () => {
  ...
  describe('createAudioUrl Helper', () => {
    test('Create synthesis audio URL with token', () => {
      ...
      expect(result).toBe(
        'https://api.example.com/file/synthesis/job123?token=audio_token'
      );
    });
    ...
    test('Create voice clone audio URL with token', () => {
      ...
      expect(localStorageMock.getItem).toHaveBeenCalledWith('access_token');
      expect(result).toBe(
        'https://api.example.com/file/voice-clone/job456?token=audio_token'
      );
    });
    ...
    test('Handle different job ID formats', () => {
      ...
      expect(createAudioUrl('abc-123')).toBe(
        'https://api.example.com/file/synthesis/abc-123?token=token'
      );
      expect(createAudioUrl('job_456')).toBe(
        'https://api.example.com/file/synthesis/job_456?token=token'
      );
    });
  });
  ...
  describe('Configuration', () => {
    ...
    test('Ensure createAudioUrl constructs URLs correctly', () => {
      ...
      expect(synthesisUrl).toBe(
        'https://api.example.com/file/synthesis/job456?token=token123'
      );
      expect(voiceCloneUrl).toBe(
        'https://api.example.com/file/voice-clone/job789?token=token123'
      );
    });
  });
  ...
  describe('localStorage Integration', () => {
    ...
    test('Check createAudioUrl works with various token values', () => {
      ...
      testCases.forEach(({ token, expected }) => {
        ...
        const result = createAudioUrl('job123');
        expect(result).toBe(
          `https://api.example.com/file/synthesis/job123?token=${expected}`
        );
      });
    });
  });
